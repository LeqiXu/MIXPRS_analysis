...
write.table(sumstat_data,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/data/sim_data/summary_data/discover/MIX/", pop,"_sim",sim_i, "_h2", h2, "_p", p, "_rhog", rhog, "_", sample_size, "_MIX_real_all.txt"),quote=F,sep='\t',row.names=F,col.names=T)
...
for sample_size in 15K 80K; do
for pop in EAS AFR SAS AMR; do
for sim_i in {1..5}; do
for p in 0.1 0.01 0.001 5e-04; do
if [[ ! -e "/gpfs/gibbs/pi/zhao/lx94/SWIFT/data/sim_data/summary_data/subsample_data/clean/${pop}_sim${sim_i}_h2${h2}_p${p}_rhog${rhog}_${sample_size}_prune_snplist_${i}_${pop}_tune_GWAS_approxTRUE_ratio3.00_repeat4.txt" ]]; then
        sbatch <<EOT
#!/bin/bash
...
--sst_file=data/sim_data/summary_data/discover/MIX/${pop}_sim${sim_i}_h2${h2}_p${p}_rhog${rhog}_${sample_size}_MIX_real_all.txt \
--pop=${pop} \
