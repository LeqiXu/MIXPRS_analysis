...
# Create wide table with percentage signs
improvement_table_wide <- improvement_summary %>%
  select(pop, sample2, Compared_Method, avg_improvement) %>%
  pivot_wider(
    names_from = Compared_Method,
    values_from = avg_improvement
  ) %>%
  mutate(across(-c(pop, sample2), ~ paste0(.x, "%"))) %>%
  arrange(pop, sample2)

# View the table
print(improvement_table_wide)

write_xlsx(improvement_table_wide, "TableS1.xlsx")