  ...
  for (sim_i in c(1:5)){
    for (p in c(0.1, 0.01, 0.001, 5e-04)){
      for (ff in c(1:4)){
        ...
        write.table(sumstat_data,paste0("/gpfs/gibbs/pi/zhao/xz674/data/sim_data/subsample_GWAS/summary_data/SDPRX/",pop,"_sim",sim_i,"_h2",h2,"_p",p,"_rhog",rhog,"_",sample_size,"_SDPRX_real_fold",ff,".txt"),quote=F,sep='\t',row.names=F,col.names=T)
