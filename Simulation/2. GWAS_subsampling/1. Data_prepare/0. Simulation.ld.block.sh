...
for (pop in c("EAS","AFR","SAS","AMR")){
  ...
  for (geno in c("15K")){
    ...
    data = read.table(paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/data/sim_data/geno_data/",pop,"/discover/",pop,".bim"))
    colnames(data) = c("chr","snpid","nonthing","position","minor","major")
...
awk 'NR==FNR{a1[$2]=$3; a2[$2]=$4; maf[$2]=$5; next; next} {print $1 "\t" $2 "\t" $4 "\t" a1[$2] "\t" a2[$2] "\t" maf[$2]}' /gpfs/gibbs/pi/zhao/lx94/SWIFT/data/sim_data/ref_data/ldblk/${geno_type}/${pop}/${pop}_${geno}_maf_${cv}.frq /gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/data/sim_data/geno_data/${pop}/discover_validate/${pop}.bim >> /gpfs/gibbs/pi/zhao/lx94/SWIFT/data/sim_data/ref_data/${split}/${geno}/ldblk_${geno_type}_${pop,,}/snpinfo_${geno_type}_hm3
fi
