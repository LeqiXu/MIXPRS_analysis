...
prs_table_final = rbind(prs_table[["EAS"]],prs_table[["AFR"]])
prs_table_final = rbind(prs_table_final,prs_table[["SAS"]])
prs_table_final = rbind(prs_table_final,prs_table[["AMR"]])

write.table(prs_table_final,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/GLGC_PRS_r2.csv"),quote=F,sep='\t',row.names=F,col.names=T)
...
prs_table[[pop]] = data.table(pop = rep(pop,n_trait), trait = trait_list,
                       MIXPRS = rep(0,n_trait),
                       PUMAS_EN = rep(0,n_trait))
...
Trait_PUMAS_EN = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/PUMAS_EN/UKB_",trait,"_PUMAS_EN_EUR_EAS_AFR_SAS_beta_",pop,"_prs_",pop,".sscore"))
Trait_PUMAS_EN = Trait_PUMAS_EN[match(Trait_pheno_id,Trait_PUMAS_EN$IID),]
...
# prs comparison            
test_data = data.table(MIXPRS = scale(MIX_PRS$SCORE1_AVG),
                       PUMAS_EN = scale(Trait_PUMAS_EN$SCORE1_AVG))
colnames(test_data) = c("MIXPRS","PUMAS_EN")
...
# Define vectors
pop <- rep(c("AFR", "EAS", "EUR", "SAS"), each = 4)
trait <- rep(c("HDL", "LDL", "logTG", "TC"), times = 4)
PUMAS_EN_paper <- c(
  0.102, 0.083, 0.042, 0.106,
  0.134, 0.051, 0.103, 0.073,
  0.186, 0.106, 0.125, 0.097,
  0.152, 0.051, 0.114, 0.047
)
prs_table_paper <- data.frame(pop, trait, PUMAS_EN_paper, stringsAsFactors = FALSE)

prs_table_final <- merge(prs_table_final,prs_table_paper, by = c("pop","trait"))

write.table(prs_table_final,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/GLGC_PUMAS_EN_PRS_r2.csv"),quote=F,sep='\t',row.names=F,col.names=T)
