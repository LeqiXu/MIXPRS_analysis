...
write.table(prs_table_final,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/Binary_3_PRS_AUC.csv"),quote=F,sep='\t',row.names=F,col.names=T)
...
for (j in 1:5){
    ...
    prs_table[t,j+2] = glmfit_auc
}

}

write.table(prs_table,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/Binary_2_PRS_AUC.csv"),quote=F,sep='\t',row.names=F,col.names=T)