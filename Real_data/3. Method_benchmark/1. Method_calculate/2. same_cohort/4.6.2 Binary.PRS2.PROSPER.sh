...
--pheno_tuning ${ukb_pheno}/pheno_data/${trait}/${trait}_${pop}_doubleid.tsv \
--NCORES 5
fi
...
--pheno_tuning ${ukb_pheno}/pheno_data/${trait}/split/${trait}_${pop}_val_${s}_doubleid.tsv \
--NCORES 5
fi
EOT
fi
...
for s in {1..5}; do
for trait in CAD LuC; do
if [[ ! -e "${path_result}/${trait}/PROSPER_val_${s}_EUR_EAS/before_ensemble/score_param.txt" ]]; then
        sbatch <<EOT
#!/bin/bash
#SBATCH --partition=scavenge,day,week
#SBATCH --requeue
...
#SBATCH --time=24:00:00
#SBATCH --mail-type=ALL
#SBATCH --job-name=${trait}_EUR_EAS_PROSPER_val_${s}
#SBATCH --output=out_${trait}_EUR_EAS_PROSPER_val_${s}.txt
...
--PATH_package ${package} \
--PATH_out ${path_result}/${trait}/PROSPER_val_${s}_EUR_EAS \
--FILE_sst ${sum_data}/${trait}_EUR_inter_PROSPER.txt,${sum_data}/${trait}_EAS_inter_PROSPER.txt \
--pop EUR,EAS \
--lassosum_param ${path_result}/${trait}/lassosum2_val_${s}/EUR/optimal_param.txt,${path_result}/${trait}/lassosum2_val_${s}/EAS/optimal_param.txt \
--chrom 1-22 \
--NCORES 5
EOT
fi
...
for trait in CAD LuC; do
for pop2 in EAS; do
if [[ ! -e "${path_result}/${trait}/PROSPER_val_${s}_EUR_EAS/after_ensemble_${pop2}/PROSPER_prs_file.txt" ]]; then
        sbatch <<EOT
#!/bin/bash
#SBATCH --partition=scavenge,day,week
#SBATCH --requeue
...
#SBATCH --time=24:00:00
#SBATCH --mail-type=ALL
#SBATCH --job-name=${trait}_EUR_EAS_PROSPER_SL_val_${s}
#SBATCH --output=out_${trait}_EUR_EAS_PROSPER_SL_val_${s}.txt
...
for (trait in c("CAD")){
for (pop2 in c("EAS")){
beta_df = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/Final_weight/same_cohort/PROSPER/",trait,"_PROSPER_val_",s,"_EUR_EAS_beta_",pop2,".txt"))
beta_df = beta_df[,c("rsid","a1","weight")]

write.table(beta_df, file=paste0("/gpfs/gibbs/pi/zhao/lx94/JointPRS/revision/result/summary_result/Final_weight/same_cohort/PROSPER/",trait,"_PROSPER_val_",s,"_EUR_EAS_beta_",pop2,".txt"), 
