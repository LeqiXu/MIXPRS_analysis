...
job_file="/gpfs/gibbs/pi/zhao/lx94/SWIFT/code/real_data/method/Final_weight/subsample_prune_PAGE_MIXPRS_weight.txt"
> $job_file  # Empty the job file if it already exists
...
dsq --job-file /gpfs/gibbs/pi/zhao/lx94/SWIFT/code/real_data/method/Final_weight/subsample_prune_PAGE_MIXPRS_weight.txt --partition=scavenge,day --requeue --mem=30G --cpus-per-task=1 --ntasks=1 --nodes=1 --time=24:00:00 --mail-type=ALL
sbatch dsq-subsample_prune_PAGE_MIXPRS_weight-$(date +%Y-%m-%d).sh
