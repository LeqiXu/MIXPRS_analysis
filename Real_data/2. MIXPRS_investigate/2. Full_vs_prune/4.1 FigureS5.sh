...
colnames(r2_table_full) = c("trait","target_pop","subsample_full_r2")

r2_table_prune = r2_table[which(r2_table$GWAS_type == "subsample_prune"),c("trait","target_pop","weight_r2")]
colnames(r2_table_prune) = c("trait","target_pop","subsample_prune_r2")

r2_full_prune = merge(r2_table_full,r2_table_prune, by = c("trait","target_pop"))
r2_full_prune$prune_improve_over_full = (r2_full_prune$subsample_prune_r2 - r2_full_prune$subsample_full_r2) / r2_full_prune$subsample_full_r2 * 100
r2_full_prune = r2_full_prune[,c("trait","target_pop","prune_improve_over_full")]
...
colnames(AUC_table_full) = c("trait","target_pop","subsample_full_AUC")

AUC_table_prune = AUC_table[which(AUC_table$GWAS_type == "subsample_prune"),c("trait","target_pop","weight_AUC")]
colnames(AUC_table_prune) = c("trait","target_pop","subsample_prune_AUC")

AUC_full_prune = merge(AUC_table_full,AUC_table_prune, by = c("trait","target_pop"))
AUC_full_prune$prune_improve_over_full = (AUC_full_prune$subsample_prune_AUC - AUC_full_prune$subsample_full_AUC) / AUC_full_prune$subsample_full_AUC * 100
AUC_full_prune = AUC_full_prune[,c("trait","target_pop","prune_improve_over_full")]

## full_table
prs_relative = rbind(r2_full_prune,AUC_full_prune)
prs_average <- prs_relative %>%
  group_by(target_pop) %>%
  summarize(mean_improvement = mean(prune_improve_over_full, na.rm = TRUE), .groups = "drop")
...
EAS_prs_relative = EAS_prs_relative %>% arrange(prune_improve_over_full)
EAS_prs_relative$trait = factor(EAS_prs_relative$trait, levels = unique(EAS_prs_relative$trait))
EAS_p <- ggplot(EAS_prs_relative, aes(x = trait, y = prune_improve_over_full)) +
  geom_bar(stat = "identity", 
           position = "dodge",
           aes(fill = prune_improve_over_full < 0)) + 
  geom_text(data = prs_average[which(prs_average$target_pop=="EAS"),], aes(x = 14/3, y = 20, 
                                    label = paste("Average increase percentage:", sprintf("%.2f%%", mean_improvement))), 
            vjust = -4, size=2.6, 
            fontface = "bold") + 
  scale_y_continuous(labels = scales::percent_format(scale = 1, accuracy = 1),
                     name = expression(paste("R"^2, scriptstyle("Prune"), "/", "R"^2, scriptstyle("Full"), "-1")), 
                     limits = c(-110,110)) +
  scale_fill_manual(values = c("#FB9A99", "#1F78B4")) + 
  theme_classic(base_size = 15) +
  my_theme +
  theme(legend.position="none") +
  ggtitle("UKBB EAS traits") + xlab("")

AFR_prs_relative = prs_relative[prs_relative$target_pop == "AFR",]
AFR_prs_relative = AFR_prs_relative %>% arrange(prune_improve_over_full)
AFR_prs_relative$trait = factor(AFR_prs_relative$trait, levels = unique(AFR_prs_relative$trait))
AFR_p <- ggplot(AFR_prs_relative, aes(x = trait, y = prune_improve_over_full)) +
  geom_bar(stat = "identity", 
           position = "dodge",
           aes(fill = prune_improve_over_full < 0)) + 
  geom_text(data = prs_average[which(prs_average$target_pop=="AFR"),], aes(x = 7/3, y = 20, 
                                    label = paste("Average increase percentage:", sprintf("%.2f%%", mean_improvement))), 
            vjust = -4, size=2.6, 
            fontface = "bold") + 
  scale_y_continuous(labels = scales::percent_format(scale = 1, accuracy = 1),
  ...
  my_theme +
  theme(legend.position="none") +
  ggtitle("UKBB AFR traits") + xlab("")

SAS_prs_relative = prs_relative[prs_relative$target_pop == "SAS",]
SAS_prs_relative = SAS_prs_relative %>% arrange(prune_improve_over_full)
SAS_prs_relative$trait = factor(SAS_prs_relative$trait, levels = unique(SAS_prs_relative$trait))
SAS_p <- ggplot(SAS_prs_relative, aes(x = trait, y = prune_improve_over_full)) +
  geom_bar(stat = "identity", 
           position = "dodge",
           aes(fill = prune_improve_over_full < 0)) + 
  geom_text(data = prs_average[which(prs_average$target_pop=="SAS"),], aes(x = 6/3, y = 48, 
                                    label = paste("Average increase percentage:", sprintf("%.2f%%", mean_improvement))), 
            vjust = -4, size=2.6, 
            fontface = "bold") + 
  scale_y_continuous(labels = scales::percent_format(scale = 1, accuracy = 1),
  ...
  my_theme +
  theme(legend.position="none") +
  ggtitle("UKBB SAS traits") + xlab("")

AMR_prs_relative = prs_relative[prs_relative$target_pop == "AMR",]
AMR_prs_relative = AMR_prs_relative %>% arrange(prune_improve_over_full)
AMR_prs_relative$trait = factor(AMR_prs_relative$trait, levels = unique(AMR_prs_relative$trait))
AMR_p <- ggplot(AMR_prs_relative, aes(x = trait, y = prune_improve_over_full)) +
  geom_bar(stat = "identity", 
           position = "dodge",
           aes(fill = prune_improve_over_full < 0)) + 
  geom_text(data = prs_average[which(prs_average$target_pop=="AMR"),], aes(x = 6/3, y = 48, 
                                    label = paste("Average increase percentage:", sprintf("%.2f%%", mean_improvement))), 
            vjust = -4, size=2.6, 
            fontface = "bold") + 
  scale_y_continuous(labels = scales::percent_format(scale = 1, accuracy = 1),
