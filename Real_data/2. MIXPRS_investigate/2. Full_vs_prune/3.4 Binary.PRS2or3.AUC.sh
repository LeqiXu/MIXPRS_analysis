...
for (t in 1:n_trait){
for (GWAS_type in c("subsample_prune","subsample_full")){
...
if (GWAS_type == "subsample_full"){
    approx = "FALSE"
    Trait_MIXPRS = fread(paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/MIXPRS/UKB_",trait,"_MIXPRS_",GWAS_type,"_linear_weights_approx",approx,"_prs_",pop,".sscore"))
}
...
write.table(prs_table,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/Binary_3_MIX_prune_PRS_AUC_full_vs_prune.csv"),quote=F,sep='\t',row.names=F,col.names=T)
...
write.table(prs_table,paste0("/gpfs/gibbs/pi/zhao/lx94/SWIFT/result/summary_result/evaluation/Binary_2_MIX_prune_PRS_AUC_full_vs_prune.csv"),quote=F,sep='\t',row.names=F,col.names=T)