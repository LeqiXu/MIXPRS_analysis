...
r2_table = r2_table[,c("target_pop","trait","approx","weight_r2")]
...
AUC_table = AUC_table[,c("target_pop","trait","approx","weight_AUC")]
colnames(AUC_table) = c("Pop","Trait","Strategy","Metric")

## organize the final table
long_table <- rbind(r2_table,AUC_table)
long_table$Strategy <- factor(long_table$Strategy, levels = c("Identity","Reference LD"))
...
write_xlsx(reshaped_table, "TableS10.xlsx")